...
Shows your country flag, country ISO code, and IP address of your current location in the system tray. Clicking the tray icon shows an "IP changelog" with the most recent IP changes ever noticed (country, IP, and timestamp, newest first, marked `(current)`), including ones noticed right after a restart.
